...
let rec compile_expr expr =
    ...
    | Expr.Call (fun_name, args) -> 
      List.concat (List.map compile_expr args) @ [CALL (fun_name, List.length args, false)]
...
let rec compile_stmt stmt =
    ...
    | Stmt.Call (f_name, args) -> 
      List.concat (List.map compile_expr args) @ [CALL (f_name, List.length args, true)]
    | Stmt.Return pos_val -> 
      match pos_val with Some v -> 
      (compile_expr v) @ [RET true] 
    | _ -> [RET false]

let compile_definition (f_name, (args, local_vars, body)) = 
[LABEL f_name; BEGIN (f_name, args, local_vars)] @ 
(compile_stmt body) @ 
[END]

let rec compile (defs, program) =
(compile_stmt program) @ [END] @ (List.concat (List.map compile_definition defs))
