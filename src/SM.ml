...
| insn :: prg' ->
     (match insn with
      ...
      | TAG t -> let e1::st1 = stack 
        in eval env (cstack, (Value.of_int (match e1 with Value.Sexp (t', _) 
          when t' = t -> 1 | _ -> 0
        ))::st1, c) prg'
      | SWAP -> let e1::e2::st1 = stack in eval env (cstack, e2::e1::st1, c) prg'
      | ENTER xs -> let vals, st1 = 
        split (List.length xs) stack in 
